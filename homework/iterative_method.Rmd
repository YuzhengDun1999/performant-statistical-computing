...
ar_precision_matvec <- function(v, auto_corr) {
  ...
  n = length(v)
  subsup_diag = auto_corr * v
  result = (1 + auto_corr^2) * v
  result[1:(n-1)] = result[1:(n-1)] - subsup_diag[2:n]
  result[2:n] = result[2:n] - subsup_diag[1:(n-1)]
  result[1] = result[1] - auto_corr^2 * v[1]
  result[n] = result[n] - auto_corr^2 * v[n]
...
```{r}
# Fill in
## function of generating correlation matrix of AR(1) process
ar1_cor <- function(n, rho) {
    exponent <- abs(matrix(1:n - 1, nrow = n, ncol = n, byrow = TRUE) - 
                        (1:n - 1))
    rho^exponent
}
Sigma = ar1_cor(ar_length, auto_corr)

result_eigs_sym = eigs(Sigma, 10)
result_eigen = eigen(Sigma, symmetric = TRUE)

result = data.frame(eigs_sym = result_eigs_sym$values, eigen = result_eigen$values[1:10])
knitr::kable(result)
```
