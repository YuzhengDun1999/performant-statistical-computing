...
The function `determine_precision` is trying to figure out the interval of the precision.
```{r}
...
U = runif(n_draw) # generate n U(0,1) realizations
I = duplicated(U) # logical(n) indicating duplicated values
C = sum(I) # number of duplicated values
est_num_expected_duplicate(n_draw,n_unique_double)

```
Not agree. The doubles are not equally spaced in the unit interval.
...
The outputs does not agree with what I expected.
The precision is between 1.11e-16 and 2.22e-16. The difference between $f(x + \Delta x) - f(x - \Delta x)$ depends on $\Delta x^3$ and the precision. After the $\Delta x^3$ is small enough, the precision is dominated. The error will grow latter.
